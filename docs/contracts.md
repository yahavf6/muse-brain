...
| `update` | `{id, rev, title?, why?, status?, confidence?, alternatives?, evidence?, files?, project?}` | `{id, rev, footer}`; stale rev -> error `conflict: #id is at rev N`; refuses every field change on an approved rule (propose a new rule that supersedes it instead); refuses a rule status change at any approval state (only `approve_rule`/`supersedes` move it) |
