...
src/schema.sql
src/db.ts             open ~/.brain/brain.db, apply schema, daily online backup to ~/.brain/backups (keep 14; since v2 the folder sits next to BRAIN_DB)
...
src/server.ts         node:http on 127.0.0.1:4747: /mcp, /api/graph, /api/version, /api/recall, /api/pre, / (static); v2 adds an optional second listener, see "V2: cloud agents"
...
- One process, two listeners (`src/server.ts`). `makeHandler('loopback' | 'public')` builds both handlers. The loopback listener (`BRAIN_PORT`, default 4747, `127.0.0.1`) is unchanged by public mode: every route, Host/Origin validation, `?agent=` identity, so local agents, hooks, the graph page, `install.ts` and `setup.ts` keep working untouched. The public listener exists only when `BRAIN_PUBLIC_PORT` is set (`publicPort()`: empty or unset = off; it must be a valid port different from `BRAIN_PORT` or startup exits 1). It binds `BRAIN_HOST` (default `0.0.0.0`) and serves only `/mcp`, `POST /api/v1/<verb>` (through `handleRest()`, same `callVerb()` path for every non-`ui_only` verb) and `GET /api/v1/openapi.json` (`openApiDoc()`, unauthenticated, also served on the loopback listener). Everything else is a 404 before any processing, and no header, `Host` or socket address is consulted: the listener a request arrived on is the boundary. Plain HTTP; TLS is the operator's job. `SIGTERM` closes both.
- `src/verbs.ts`: `whoIs(req, { bearer })`; `bearer: true` (the public listener) reads only `Authorization: Bearer` (`?agent=` is ignored) and throws `AuthError` on a missing or unknown token, which `requestWho()` in `server.ts` turns into a 401 with `WWW-Authenticate: Bearer`. `agentWho(agent)` builds `scope: 'full'` plus the `agent_policy` row, shared by both listeners and by the hook's server-internal `claude-code` identity, so handlers still only ever see `{agent, scope}`. `callVerb()` refuses every `ui_only` verb for a non-admin caller, so the "single enforcement point" claim now holds for those too.
- `src/tokens.ts`: tokens are 32 random bytes as hex, stored as sha256 -> `{agent, created_at}` in `BRAIN_TOKENS_FILE` (default `~/.brain/tokens.json`), shown once at mint. Writes are atomic (a fresh 0600 temp file renamed over the old one, so the mode is always 0600). `verifyToken()` hardcodes `scope: 'full'`, so a token can never become `admin`. A malformed file is never rewritten: mint/revoke throw, verify fails closed.
- `handleMcp()` builds a new `McpServer` and transport for every `/mcp` request and passes the body, read through the 1 MB cap, to the transport pre-parsed.
- `src/db.ts`: `backupDaily()` writes to `<dir of BRAIN_DB>/backups`, so on a volume mounted at `/data` the snapshots land in `/data/backups` and survive the container.
- `scripts/token.ts` (`npm run token -- mint|list|revoke`): the only way to manage tokens, and it runs inside a slim container with no curl and no running server. `mint <agent> --read-only` also upserts an `agent_policy` row (read on, write off).
- `Dockerfile` (`BRAIN_PORT=4748`, `BRAIN_PUBLIC_PORT=4747`, node as PID 1 so SIGTERM reaches the server, a node-based `HEALTHCHECK` on `/api/v1/openapi.json`), `.dockerignore`, `fly.toml` (same env, an HTTP health check), `.do-marketplace/cloud-init.yaml` (publishes on the Droplet's loopback, refuses to start without the volume mounted): the deploy files. `docs/muse-connector-brief.md`: the agent-facing REST brief for Meta Muse's Custom Connector. Tests: `test/tokens.test.ts`, `test/rest.test.ts`, `test/public.test.ts`.
...
- **Two listeners, not one listener with a local-only heuristic.** The first build had a single listener that an on/off env flag switched into public mode, and kept the admin routes local by checking the socket address for loopback and the request for five proxy forwarding headers. A default nginx `proxy_pass` on the same machine connects from loopback and sends no forwarding header, so that check failed open and exposed `/api/call` (admin scope) to the internet. Review on 2026-09-23 replaced it with a second listener: the public one has a three-route allow-list and nothing else exists on it, so no header can argue a request into an admin route. The loopback listener is untouched, which also means local agents and hooks needed no token and no config change. The same review found and fixed a shared `McpServer` that crossed responses between concurrent `/mcp` requests (one server per request now).
- **Fly.io is the primary deploy.** `fly.toml` mounts one volume at `/data` (DB, logs, tokens file, backups), forces HTTPS, keeps one machine running (`auto_stop_machines = false`, `min_machines_running = 1`) and health-checks `GET /api/v1/openapi.json`. Exactly one machine: a Fly volume pins the app to one machine in one region, and a second machine would get its own empty volume.
- **A DigitalOcean Droplet, not App Platform** (its disk is ephemeral, so every redeploy would wipe the database) **and not Vercel** (stateless functions cannot hold a sqlite file). The container has no TLS in front of it, so cloud-init publishes it on the Droplet's loopback only; the operator adds a TLS proxy (Caddy) in front of `127.0.0.1:4747`, and any proxy works because the public listener consults no forwarded headers.
- **Cloudflare Tunnel from a Mac** is the no-cost alternative: add `BRAIN_PUBLIC_PORT=4748` (and `BRAIN_HOST=127.0.0.1`, so only the tunnel can reach it) to `~/.brain/.env`, restart the service, mint a token, and point `cloudflared tunnel --url http://127.0.0.1:4748` at it. Local agents keep using the loopback listener with no token. Not shipped as a script and not tested here; it only works while the Mac is awake.
