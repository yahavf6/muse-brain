...
<p align="center">
  ...
  <img alt="runtime deps" src="https://img.shields.io/badge/runtime%20deps-3-lightgrey">
...
<p align="center">
  <img src="docs/media/hero.gif" width="1000" alt="Present-mode auto-rotate of the 3D decision graph">
</p>
...
```json
{
  "intent": "history",
  "hits": [
    { "id": 18, "kind": "action", "title": "Changed dashboard default sort from name to most-recent", "score": 0.88 },
...
<p align="center">
  <img src="docs/media/inspector.png" width="900" alt="Inspector showing a refuted thought with its incoming refutes edge">
</p>

The refuted thought, above, now shows its incoming edge in the inspector. The agent proposes a rule so the mistake is not repeated:
...
Next session, `SessionStart` injects the approved rule scoped to this repo, and the next time an `Edit` touches `defaultSort`, `PreToolUse` asks the server and the guard on rule #43 denies the call before it runs.
...
3. **Invariants live in the store, not in prompts.** Four kinds, nine edges, eight triggers. A prompt can be ignored; a `CHECK` constraint cannot.
...
|---|---|---|---|---|---|---|---|---|
| **Muse Brain** | 4 typed nodes: thought, action, rule, conclusion | No | Yes, 9 edges, DB triggers | Yes, guards deny / ask / allow | Yes, `evaluates` edge | Yes, one MCP server (hooks: Claude Code only) | SQLite, 3 deps | PolyForm Noncommercial |
| mem0 | Flat text facts + vectors | Yes by default[^mem0-1] | Partial, graph memory is Platform-only[^mem0-2] | No | No, manual feedback API[^mem0-3] | Partial, scoping by `agent_id` / `app_id` | Vector DB, Qdrant default + 20 options | Apache-2.0 |
| Zep / Graphiti | Temporal fact graph: episodes, entities, edges[^zep-1] | Yes, `add_episode` extraction[^zep-2] | Yes, but untyped fact edges, not thought / action / rule | No | No | Partial, group graphs, Zep ABAC | Neo4j / FalkorDB / Neptune (OSS); proprietary (Zep cloud) | Apache-2.0 (Graphiti) |
| Letta | Free-text memory blocks + vector passages | No separate extraction, the agent LLM is the writer[^letta-1] | No, no graph | No, `read_only` blocks only | No | Yes, shared blocks / shared MemFS repos[^letta-2] | Postgres + pgvector; MemFS = git repo per agent | Apache-2.0 |
| claude-mem | Flat typed observation rows | Yes, Agent SDK worker on every `PostToolUse` | No | No | No | Yes, same DB across harnesses[^cm-1] | SQLite + FTS5, optional Chroma | Apache-2.0 |
...
| Cognee | LLM-extracted knowledge graph + vectors | Yes, `cognify()`[^cog-1] | Yes, incl. `contradicts` edges | No | Partial, 1 to 5 feedback weights[^cog-2] | Partial, shared datasets / tenants; default store not concurrency-safe | Kuzu + LanceDB + SQLite | Apache-2.0 |
| Supermemory | Temporal fact graph: updates / extends / derives | Yes, async extraction queue[^sm-1] | Yes | Partial, review queue for low-confidence inferred memories, not rules[^sm-2] | No | Yes, shared `containerTag` | Cloudflare Workers + Postgres / pgvector | MIT (local binary capped at 10k docs) |
| Honcho | Peer representations: premises + conclusions | Async, deriver runs after the request commits[^ho-1] | No entity graph | No | No | Yes, native multi-peer | Postgres + pgvector, Redis | AGPL-3.0 |

[^mem0-1]: `infer=False` bypasses extraction; on by default. https://docs.mem0.ai/core-concepts/memory-operations/add
[^mem0-2]: Graph memory is a Platform-only feature. https://docs.mem0.ai/open-source/graph_memory/overview
...
Muse Brain is a decision graph, not a chat-recall system, so LoCoMo and LongMemEval measure something it does not attempt: reconstructing facts from a long conversation. Those benchmarks also have real problems. An independent audit found 6.4% of LoCoMo's golden answers corrupted, capping any system near 93.6%. Judge choice alone swings identical answers from 32% to 84%. mem0 and Zep have disputed each other's LoCoMo numbers for over a year, still unresolved. And a third-party, cost-aware run found plain RAG at $0.65 beating two graph-based systems that cost several times more. None of that says graph memory is bad; it says a recall leaderboard is not the yardstick for a system built to hold why, not what was said.
...
The author ran claude-mem for months before building this: the local store grew to 101,485 observations and not one of them was ever linked to an outcome or fed back into a later decision. Closing that gap is what this project exists to do.
...
| `get` | one node with its edges | MCP + API |
...
| `update` | change status, verdict, confidence or props, with a `rev` check | MCP + API |
...
Open the page, click **Connect agent**, then **Install** next to a client. It writes that client's MCP config directly (Claude Code's `~/.claude.json` plus hook groups, Codex's `~/.codex/config.toml`, Cursor's `~/.cursor/mcp.json`, Gemini CLI's `~/.gemini/settings.json`, Claude Desktop's config via `mcp-remote`), backing up any file it touches first as `<file>.bak-brain-<timestamp>`. A client that is already wired up is left untouched: no write, no backup.
...
BRAIN_JEV_GUARDS=shadow      # shadow | on
BRAIN_GUARDS=off             # kill switch for all guards
```

Also read: `BRAIN_PORT` (default 4747), `BRAIN_DB` (default `~/.brain/brain.db`).
...
Further reading: [`docs/contracts.md`](docs/contracts.md) (the wire format: every verb signature, endpoint, env var), [`docs/design.md`](docs/design.md) (the original internal design note, kept as history), [`skills/brain/SKILL.md`](skills/brain/SKILL.md) (the agent-facing protocol), [`PRODUCT.md`](PRODUCT.md), [`DESIGN.md`](DESIGN.md).
